    ...
    <div class="py-12">
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
          <!-- Invoice Header -->
          <div class="mb-6 grid grid-cols-4 gap-4" v-if="invoiceItems.data.length > 0">
            <!-- Invoice Info - Column 1 -->
            <div class="space-y-2">
              <h3 class="text-lg font-semibold">Invoice Info</h3>
              <div class="text-lg space-y-1">
                <p><span class="font-medium">ISP:</span> {{ invoiceItems.data[0].invoice.isp?.name }}</p>
                <p><span class="font-medium">Bill Period:</span> {{ invoiceItems.data[0].invoice?.temp_bill?.billing_period }}</p>
                <p><span class="font-medium">Status:</span> {{ invoiceItems.data[0].invoice?.temp_bill?.status }}</p>
              </div>
            </div>
            ...
            <div class="space-y-2">
              ...
              <div class="grid grid-cols-2 gap-2">
                <div class="bg-blue-50 rounded p-2 text-center">
                  ...
                  <p class="text-lg font-semibold text-blue-700">{{ invoiceItems.data[0].invoice.total_mrc_customer }}</p>
                </div>
                <div class="bg-blue-50 rounded p-2 text-center">
                    <p class="text-xs text-blue-600">Total MRC Amount</p>
                    <p class="text-lg font-semibold text-blue-700">{{ formatNumber(invoiceItems.data[0].invoice.total_mrc_amount) }}</p>
                  </div>
                <div class="bg-green-50 rounded p-2 text-center">
                  <p class="text-xs text-green-600">New</p>
                  <p class="text-lg font-semibold text-green-700">{{ invoiceItems.data[0].invoice.total_new_customer }}</p>
                </div>
               
                  <div class="bg-green-50 rounded p-2 text-center">
                    <p class="text-xs text-green-600">Total New Amount</p>
                    <p class="text-lg font-semibold text-green-700">{{ formatNumber(invoiceItems.data[0].invoice.total_installation_amount) }}</p>
                  </div>
                  <div class="bg-yellow-50 rounded p-2 text-center">
                    <p class="text-xs text-yellow-600">Additional</p>
                    <p class="text-lg font-semibold text-yellow-700">{{ invoiceItems.data[0].invoice.additional_description }}</p>
                  </div>
                 
                    <div class="bg-yellow-50 rounded p-2 text-center">
                      <p class="text-xs text-yellow-600">Total New Amount</p>
                      <p class="text-lg font-semibold text-yellow-700">{{ formatNumber(invoiceItems.data[0].invoice.additional_fees) }}</p>
                    </div>
            ...
            <div class="space-y-2 col-span-2">
              ...
              <div class="bg-gray-50 rounded p-2 text-sm">
                ...
                <div v-for="invoice in invoices" 
                  ...
                  <div class="col-span-2 truncate">{{ invoice.category }}</div>
                  <div class="text-center">{{ invoice.total_customers }}</div>
                  <div class="text-right">{{ formatNumber(invoice.unit_price) }}</div>
                  <div class="text-right font-medium">{{ formatNumber(invoice.total_amount) }}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="mt-8">
            ...
            <div class="overflow-x-auto">
             ...
             <!-- Pagination -->
          <div class="mt-4">
            <Pagination :links="invoiceItems.links" />
          </div>
          </div>
          ...
          <div class="mt-6 flex justify-end space-x-3">
            <Link
            :href="route('showbill.show', { bill_id: invoiceItems.data[0].invoice.bill_id })"
...
import EditInvoiceItem from '@/Pages/Client/EditInvoiceItem.vue'
...
export default defineComponent({
  ...
  computed: {
    filteredItems() {
      return this.invoiceItems.data.filter(item => {
  ...
  methods: {
    ...
    doExcel() {
      let parm = Object.create({});
        parm.id = this.invoiceItems.data[0]?.invoice_id;

      axios.post("/exportBillingItemExcel",  parm, { responseType: "blob" }).then((response) => {
        ...
        link.download = `billings_${this.invoiceItems.data[0].invoice.isp?.name}_${this.invoiceItems.data[0]?.invoice?.bill?.bill_number}_${new Date().toISOString().split('T')[0]}.xlsx`;
        link.click();
      });
    },
  },
