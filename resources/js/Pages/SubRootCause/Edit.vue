...
const form = useForm({
  root_cause_id: props.subRootCause.root_cause_id,
  name: props.subRootCause.name,
  description: props.subRootCause.description || '',
  status: props.subRootCause.status
});

const submit = () => {
  form.put(route('sub-root-causes.update', props.subRootCause.id));
};
</script>

<template>
  <AppLayout title="Edit Sub Root Cause">
    <template #header>
