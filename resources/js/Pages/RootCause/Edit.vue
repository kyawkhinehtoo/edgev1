...
const form = useForm({
  name: props.rootCause.name,
  description: props.rootCause.description || '',
  status: props.rootCause.status,
  is_installation: props.rootCause.is_installation || false,
  is_maintenance: props.rootCause.is_maintenance || false,
  is_pending: props.rootCause.is_pending || false,
});

const submit = () => {
  form.put(route('root-causes.update', props.rootCause.id));
};
</script>

<template>
  <AppLayout title="Edit Root Cause">
    <template #header>
