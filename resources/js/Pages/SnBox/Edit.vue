...
const form = useForm({
  name: props.snBox.name,
  dnSplitter: props.dnSplitters.find(splitter => splitter.id === props.snBox.dn_splitter_id),
  dn_splitter_id: props.snBox.dn_splitter_id,
  location: props.snBox.location,
  status: props.snBox.status
})

const submit = () => {
  form.put(route('sn-boxes.update', props.snBox.id), {
    onSuccess: (page) => {
        Toast.fire({
        icon: 'success',
